 ...
 * @param {string} [acceleration=65] - Determines the acceleration given
 * to the entity when pressing the keys
 * @param {number} [enabled=true] - To completely enable or disable the controls
 * @param {number} [fly=false] - Determines if the direction of the movement sticks
 ...
 * @param {string} [adAxis='z'] - The axis that the A and D keys operate on
 * @param {string} [wsInverted=false] - WS Axis is inverted
 * @param {string} [adInverted=false] - AD Axis is inverted
 */
module.exports.Component = registerComponent('wasd-controls', {
  ...
  update: {
    value: function (previousData) {
      ...
      // If data has changed we reset the velocity
      if (previousData || delta > 0.2) {
        velocity[adAxis] = 0;
        velocity[wsAxis] = 0;
        return;
      }
